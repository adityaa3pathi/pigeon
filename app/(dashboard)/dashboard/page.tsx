import { getFriendsByUserId } from '@/app/helpers/get-frineds-by-user-d'
import { fetchRedis } from '@/app/helpers/redis'
import { authOptions } from '@/app/libs/auth'
import { chatHrefConstructor } from '@/app/libs/utils'
  ...
  const friendsWithLastMessage = await Promise.all(
    friends.map(async (friend) => {
      const [lastMessageRaw] = await fetchRedis(
        'zrange',
        `chat:${chatHrefConstructor(session.user.id, friend.id)}:messages`, -1,
        -1
      ) as string[]
  ...
  return <div className='container py-12'>
        ...
        {friendsWithLastMessage.length === 0 ? (
          ...
          <div key={friend.id} className='relative bg-zinc-50 border border-zinc-200 p-3 rounded-md'>
              <div className='absolute right-4 isnet-y-0 flex items-center'>
